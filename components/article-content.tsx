...
const mdxComponents = {
  ...
  p: ({ children, ...props }: ParagraphProps) => {
    const hasBlockElements = React.Children.toArray(children).some((child) => {
      if (React.isValidElement(child)) {
        ...
        return ["div", "section", "article", "aside", "nav", "header", "footer", "main"].includes(type)
      }
      return false
    })
  ...
  blockquote: ({ children, ...props }: BlockquoteProps) => (
    <blockquote
      className="bg-muted/30 my-6 py-2 pl-4 border-primary border-l-4 text-muted-foreground italic"
      {...props}
    >
      {children}
    </blockquote>
  ...
  img: ({ src, alt, ...props }: ImgProps) => (
    <img src={src || "/placeholder.svg"} alt={alt} className="shadow-md my-6 rounded-lg max-w-full h-auto" {...props} />
  ),
